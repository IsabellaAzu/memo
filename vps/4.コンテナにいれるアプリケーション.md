...
```command
# コンテナイメージ取得
$ sudo docker pull ruby:2.4.1-alpine
# コンテナ起動：コンテナ名yyyで、dockerの/optにRailsプロジェクトがmountされる。
$ sudo docker run -it -p 3000:3000 -v /path/to/rails/xxx:/opt --name yyy ruby:2.4.1-alpine /bin/sh
# 
$ cd /path/to/rails/xxx
$ sudo vim Gemfile
 ↓
...
bundle install で nio4r のインストールに失敗  
https://pinzolo.github.io/2016/09/29/bundle-install-error-nio4r.html  
