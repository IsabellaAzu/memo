...
Dockerfileを使用したDockerイメージの作成(手動でApache、PHPをインストール)  
http://www.memotansu.jp/2016/08/docker/236/  
Dockerコマンドメモ  
...
http://qiita.com/tukiyo3/items/9444fc67153785f354c6  
...
$ sudo docker run -it -p 8080:80 nginx:alpine /bin/sh
```

#### (2-3) 設定（もう１回コンテナイメージから作ってみよう！！）
...
$ mkdir /www
$ chown -R www:www /var/lib/nginx # これできないかも
$ chown -R www:www /www
...
# nginxの起動
    # rc-serviceコマンドが無い場合
    # 「/bin/sh: rc-service: not found」のerror文言
    # 解決方法 : https://github.com/gliderlabs/docker-alpine/issues/183
    $ apk add openrc --no-cache
$ rc-service nginx start # ←　これだと、error : 「rc-service: service `nginx' does not exist」
$ nginx # これで起動したっぽい（要調査）
```
※[GECOS field](https://uc2.h2np.net/index.php/GECOS%E3%83%95%E3%82%A3%E3%83%BC%E3%83%AB%E3%83%89)  



#### (2-x) alpineベースのコンテナに、apk(パッケージ関連のコマンド)でnginxをソースコードからインストールする場合
